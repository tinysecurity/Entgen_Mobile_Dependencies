  ...
  struct EnrollmentConfig {
    ...
    int             topicsBoolCount;
    char clientCertPem[MAX_CERT_LEN];   // ADD
    char clientKeyPem[MAX_CERT_LEN];    // ADD
  ...
  uint32_t memoryCanary = 0xDEADBEEF;
  ...
  void setup() {
   ...
   if (!partitionCheck()) {
    ...
    mqttClient.setTxPayloadSize(600);
     Serial.print("[DEBUG] Canary value: 0x");
  Serial.println(memoryCanary, HEX);
    if (connectMQTT()) {
      applyTopicSubscriptions(registration);

      // ADD — tell the phone OTA is genuinely complete now that the device
      // has successfully reconnected post-reboot. Previously nothing ever
      // published the "complete" half of handleOtaTrigger()'s "flashing" status.
      {
  ...
  // ADD — simple NTP client, self-contained. Returns Unix time, or 0 on failure.
...
bool parseBoolTopicEntry(JsonObjectConst entry, TopicEntryBool &out) {
  ...
  bool loadConfigFromFlash(EnrollmentConfig &cfg) {
    ...
    // loadConfigFromFlash(), alongside the existing field reads:
ok &= copyJsonStringField(doc, "mqtt_username", cfg.mqttUsername, MAX_USERNAME_LEN, true);
  ...
  /*
bool connectMQTT() {
  // -- Bare-bones, mirrors the probe's inline sequence exactly --
  // Removed for this test: Will/LWT setup, subscription logic (still
  // called separately by applyTopicSubscriptions() after this returns
  // true), and the retry-loop's verbose per-attempt error printing
  // beyond what's needed to see the same diagnostics the probe shows.

  sslClient.setClient(&wifiClient, true);
  sslClient.setDebugLevel(3);
  ...
  if (now > 0) {
    ...
    Serial.println("[MQTT] No NTP time — cert validity check may fail.");
  }
