  ...
  #define CONFIG_TEMP_PATH "/littlefs/config.json.tmp"
  ...
  void setup() {
    ...
    // If the filesystem is not found, the system will
    if (!LittleFS.begin(false, "/littlefs")) {
      Serial.println("FATAL: Could not mount/format LittleFS.");
    ...
    WiFi.config(registration.localIP_wifi, registration.gateway, registration.subnet, registration.dns);
    if (!connectWiFi()) {
